...
export const MAX_FILE_SIZE_S3_ENDPOINT = 5;
export const FILE_NUMBER_LIMIT = 1;
...
export const saveFileInfoInDB = async (presignedUrls: PresignedUrlProp[]) => {
  try {
    const res = await fetch("/api/files/upload/saveFileInfo", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!res.ok) {
      throw new Error("Failed to save file info in the database");
    }
...
export const handleUpload = async (
  ...
  const res = await saveFileInfoInDB(presignedUrls);
  onUploadSuccess();
  return res;
};
